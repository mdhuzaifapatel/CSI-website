...
$("body").append(
  "<style>#theme-switcher{display: inline-block; position: fixed; top: 80px; left: 10px;z-index: 9999;} #btn-theme{padding:7px 10px;background-color:rgba(255,255,255,0.2);border-radius:7px;margin-bottom:5px;cursor:pointer;box-shadow: 0 0 10px rgba(0,0,0,0.3);}#btn-theme .fa{color:#ffffff;text-shadow: 0 0 5px rgba(0,0,0,0.7);}#btn-theme:hover{background-color:rgba(255,255,255,0.4)}.switches-holder{transform-origin:center top;transform:scaleY(0);opacity:0;padding: 7px 7px;background-color: rgba(255,255,255,0.9);border-radius: 7px;width: 34px;font-size: 0;transition: all 0.3s cubic-bezier(0.34, 1.61, 0.7, 1);box-shadow: 0 0 10px rgba(0,0,0,0.3);}.switches-holder.active{transform:scaleY(1);opacity:1;}.switches-holder .switch{display:inline-block;width:20px;height:20px;margin-bottom:5px;border-radius:4px;cursor:pointer;}.switches-holder .switch.active{box-shadow:inset 0 0 2px 2px rgba(0,0,0,0.3)} " +
    setCssColors +
    '.switches-holder div:last-child{margin-bottom:0;}</style><div id="theme-switcher"><div id="btn-theme"><i class="fa fa-paint-brush" aria-hidden="true"></i></div><div class="switches-holder">' +
    setDivs +
    "</div></div>"
);
...
if (whmcs) {
  $("head").append(
    '<link id="custom-css2" rel="stylesheet" type="text/css" href="">'
  );
}
$("head").append(
  '<link id="custom-css1" rel="stylesheet" type="text/css" href="">'
);
...
for (var i = 0; i <= themes.length - 1; i++) {
  if (themeSelected == themes[i]) {
    if (themeSelected == "org") {
      ...
      if (whmcs) {
        $("head #custom-css1").attr(
          "href",
          "templates/hostify/css/style-" + themes[i] + ".css"
        );
        $("head #custom-css2").attr(
          "href",
          "templates/hostify/css/styles-modified-" + themes[i] + ".css"
        );
      } else {
...
for (var i = 0; i <= themes.length - 1; i++) {
  if (Number(i + 1) == 1) {
    $(".switches-holder .switch" + Number(i + 1), "#theme-switcher").on(
      "click",
      function () {
        if (whmcs) {
          $("head #custom-css1").attr("href", "");
        ...
        document.cookie =
          "hostifythemeSet=" + themes[Number($(this).data("num"))];
    ...
    $(".switches-holder .switch" + Number(i + 1), "#theme-switcher").on(
      ...
      function () {
        if (whmcs) {
          $("head #custom-css1").attr(
            "href",
            "templates/hostify/css/style-" +
              themes[Number($(this).data("num"))] +
              ".css"
          );
          $("head #custom-css2").attr(
            "href",
            "templates/hostify/css/styles-modified-" +
              themes[Number($(this).data("num"))] +
              ".css"
          );
        } else {
          $("head #custom-css1").attr(
            "href",
            "css/style-" + themes[Number($(this).data("num"))] + ".css"
          );
        }
        document.cookie =
          "hostifythemeSet=" + themes[Number($(this).data("num"))];
...
mainSlider.on("afterChange", function (event, slick, currentSlide, nextSlide) {
  ...
  $(".slide > div:nth-child(2)", "#main-slider").removeClass(
    "animated animation-delay1"
  );

  $(".slick-active > div:nth-child(1)", "#main-slider").addClass("animated");
  $(".slick-active > div:nth-child(2)", "#main-slider").addClass(
    "animated animation-delay1"
  );
});
...
$(window).on("load", function () {
  ...
  featureIconHolder.on("click", function () {
    ...
    $(".feature-d" + $(this).data("id"), "#features-holder").addClass(
      "show-details"
    );
  });
  ...
  for (var i = 0; i <= locationsList.length - 1; i++) {
    ...
    serversLocationHolder.append(
      '<div class="server-marker" style="top:' +
        locationsList[i][1] +
        "px;left:" +
        locationsList[i][2] +
